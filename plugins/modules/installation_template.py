...
def main(
    ...
    if state == "absent":
        if src_template in template_list:
            ...
            module.exit_json(
                msg="Template {} succesfully deleted".format(src_template), changed=True
            )
        else:
            module.exit_json(
    ...
    if src_template in template_list:
        module.exit_json(
            msg="Template {} already exists".format(src_template), changed=False
        )
    ...
    client.wrap_call(
        "PUT", f"/me/installationTemplate/{conf['templateName']}", **templates
    )
    ...
    client.wrap_call(
        "POST", f"/me/installationTemplate/{conf['templateName']}/checkIntegrity"
    )
    module.exit_json(
        msg="Template {} succesfully created".format(conf["templateName"]), changed=True
    )
