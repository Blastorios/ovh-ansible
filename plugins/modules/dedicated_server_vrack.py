...
def main(
    ...
    result = client.wrap_call(
        "GET", f"/dedicated/server/{service_name}/virtualNetworkInterface", mode="vrack"
    )
    ...
    if len(result):
        ...
        is_already_registered = client.wrap_call(
            "GET", f"/vrack/{vrack}/dedicatedServerInterfaceDetails"
        )
        ...
        module.exit_json(
            msg="{} has been added to new {}".format(service_name, vrack), changed=True
        )

    # Old generation
    else:
        is_already_registered = client.wrap_call(
            "GET", f"/vrack/{vrack}/dedicatedServer"
        )
        ...
        for old_server in is_already_registered:
            if old_server == service_name:
                ...
                if state == "absent":
                    result = client.wrap_call(
                        "DELETE", f"/vrack/{vrack}/dedicatedServer/{service_name}"
                    )
                    module.exit_json(
        ...
        # Server is not yet registered on vrack, go for it
        client.wrap_call(
            "POST", f"/vrack/{vrack}/dedicatedServer", dedicatedServer=service_name
        )
        module.exit_json(
            msg="{} has been added to old {}".format(service_name, vrack), changed=True
        )
